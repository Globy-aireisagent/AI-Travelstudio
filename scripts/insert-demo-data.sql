...
-- Insert demo users with correct role values
...
UPDATE feature_requests SET votes = (
  SELECT COUNT(*) * CASE WHEN vote_type = 'up' THEN 1 ELSE -1 END
  FROM feature_votes 
  WHERE feature_votes.feature_id = feature_requests.id
  GROUP BY vote_type
) WHERE id IN (
  SELECT DISTINCT feature_id FROM feature_votes
);
